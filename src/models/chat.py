...
def _create_llm_from_config(
    ...
    if not configuration_key:
        ...
        raise ValueError(f"Unknown LLM type: {llm_type}. Supported types: {supported_types}")
    ...
    if not isinstance(llm_specific_config, dict):
        raise TypeError(f"Invalid LLM configuration for {llm_type}: expected dict, got {type(llm_specific_config)}")
    ...
    if llm_type == "reasoning":
        ...
        return ChatOpenAIReasoning(**merged_config,  extra_body={"chat_template_kwargs": {"thinking":True}})
    else:
        ...
        return ChatOpenAI(**merged_config,  extra_body={"chat_template_kwargs": {"thinking":False}})
...
def get_configured_llm_models() -> Dict[str, list[str]]:
    ...
    try:
        ...
        if loaded_config is None:
            print(f"Warning: Could not load configuration from {configuration_file_path}")
            return {}
        ...
        for llm_type in get_args(LLMType):
            ...
            configuration_key = llm_type_config_mapping.get(llm_type, "")
            yaml_configuration = loaded_config.get(configuration_key, {}) if configuration_key else {}
