...
class ChatRequest(BaseModel):
    ...
    thread_id: str = Field(
        "__default__", description="Unique conversation thread identifier"
    )
    max_plan_iterations: int = Field(
        1, ge=1, le=10, description="Maximum planning iterations allowed"
    )
    max_step_num: int = Field(
        3, ge=1, le=20, description="Maximum steps per execution plan"
    )
    max_search_results: int = Field(
        3, ge=1, le=50, description="Maximum search results to retrieve"
    )
    auto_accepted_plan: bool = Field(
        False, description="Automatically accept generated execution plan"
    )
    interrupt_feedback: Optional[str] = Field(
        None, description="User feedback for plan interruption/modification"
    )
    mcp_settings: Optional[Dict] = Field(
        None, description="Model Context Protocol configuration settings"
    )
    enable_background_investigation: bool = Field(
        True, description="Enable preliminary research before planning"
    )
    report_style: ReportStyle = Field(
        ReportStyle.ACADEMIC, description="Output report formatting style"
    )
    enable_deep_thinking: bool = Field(
        False, description="Enable enhanced reasoning and analysis"
    )
...
class TTSRequest(BaseModel):
    ...
    text: str = Field(
        ..., min_length=1, description="Text content for speech synthesis"
    )
    voice_type: str = Field("BV700_V2_streaming", description="Voice model identifier")
    encoding: str = Field("mp3", description="Audio output encoding format")
    speed_ratio: float = Field(
        1.0, ge=0.5, le=2.0, description="Speech speed multiplier (0.5-2.0)"
    )
    volume_ratio: float = Field(
        1.0, ge=0.0, le=2.0, description="Speech volume multiplier (0.0-2.0)"
    )
    pitch_ratio: float = Field(
        1.0, ge=0.5, le=2.0, description="Speech pitch multiplier (0.5-2.0)"
    )
...
class GeneratePodcastRequest(BaseModel):
    ...
    content: str = Field(
        ..., min_length=1, description="Source content for podcast generation"
    )
...
class GeneratePPTRequest(BaseModel):
    ...
    content: str = Field(
        ..., min_length=1, description="Source content for presentation generation"
    )
...
class GenerateProseRequest(BaseModel):
    ...
    prompt: str = Field(..., min_length=1, description="Writing prompt or topic")
    option: str = Field(..., description="Writing style and format option")
    command: str = Field("", description="Custom instructions for prose generation")
...
class EnhancePromptRequest(BaseModel):
    ...
    prompt: str = Field(
        ..., min_length=1, description="Original prompt text to enhance"
    )
    context: str = Field("", description="Additional context for prompt enhancement")
    report_style: str = Field(
        "academic", description="Target output style for enhanced prompt"
    )
