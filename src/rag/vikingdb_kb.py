...
class VikingDBKnowledgeBaseProvider(Retriever):
    ...
    def __init__(self) -> None:
        ...
        if not api_url:
            raise ValueError(
                "VIKINGDB_KNOWLEDGE_BASE_API_URL environment variable is not set"
            )
        ...
        if not api_ak:
            raise ValueError(
                "VIKINGDB_KNOWLEDGE_BASE_API_AK environment variable is not set"
            )
        self.api_ak = api_ak
        ...
        if not api_sk:
            raise ValueError(
                "VIKINGDB_KNOWLEDGE_BASE_API_SK environment variable is not set"
            )
        self.api_sk = api_sk
...
def parse_uri(uri: str) -> Tuple[str, str]:
    ...
    if parsed_uri.scheme != "rag":
        raise ValueError(
            f"Invalid URI scheme. Expected 'rag', got '{parsed_uri.scheme}'"
        )
    ...
    if len(path_parts) < 2 or path_parts[0] != "dataset":
        raise ValueError(
            f"Invalid URI path format. Expected '/dataset/{{resource_id}}', got '{parsed_uri.path}'"
        )
