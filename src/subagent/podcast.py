...
class PodcastDialogueLine(BaseModel):
    ...
    speaker: Literal["male", "female"] = Field(
        default="male", 
        description="Voice type for this line"
    )
    text_content: str = Field(
        default="", 
        description="Text content to be spoken"
    )
...
class PodcastScript(BaseModel):
    ...
    locale: Literal["en", "zh"] = Field(
        default="en", 
        description="Language locale for the script"
    )
    dialogue_lines: List[PodcastDialogueLine] = Field(
        default=[], 
        description="List of script lines with speaker assignments"
    )
...
def create_volcengine_tts_client() -> VolcengineTTS:
    ...
    if not app_id:
        raise ValueError("VOLCENGINE_TTS_APPID environment variable is required but not set")
    
    access_token = os.getenv("VOLCENGINE_TTS_ACCESS_TOKEN", "").strip()
    if not access_token:
        raise ValueError("VOLCENGINE_TTS_ACCESS_TOKEN environment variable is required but not set")
...
def convert_script_to_audio(state: PodcastState) -> dict:
    ...
    for line_index, dialogue_line in enumerate(state["script"].dialogue_lines):
        ...
        voice_type = (
            "BV002_streaming" if dialogue_line.speaker == "male" 
            else "BV001_streaming"
        )
        ...
        try:
            ...
            if result.get("success"):
                ...
                logger.error(f"TTS conversion failed for line {line_index + 1}: {error_msg}")
                
        except Exception as e:
            logger.error(f"Exception during TTS conversion for line {line_index + 1}: {e}")
    
    logger.info(f"Successfully converted {successful_conversions}/{len(state['script'].dialogue_lines)} lines to audio")
...
def build_graph():
    ...
    workflow_builder.add_edge("script_writer", "tts_converter")  # Convert script to audio
...
def main() -> None:
    ...
    try:
        ...
        if final_state.get("script") and final_state["script"].dialogue_lines:
            ...
            for line_index, dialogue_line in enumerate(final_state["script"].dialogue_lines):
                speaker_prefix = "<M>" if dialogue_line.speaker == "male" else "<F>"
                print(f"{line_index + 1:2d}. {speaker_prefix} {dialogue_line.text_content}")
