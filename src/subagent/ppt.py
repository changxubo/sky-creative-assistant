...
def generate_presentation_file(state: PowerPointWorkflowState) -> Dict[str, Any]:
    ...
    try:
        ...
        if not os.path.exists(generated_file_path):
            raise FileNotFoundError(f"Expected output file was not created: {generated_file_path}")
...
def compose_presentation_content(state: PowerPointWorkflowState) -> Dict[str, Any]:
    ...
    try:
        ...
        temp_file_path = os.path.join(
            os.getcwd(), f"ppt_content_{uuid.uuid4().hex}.md"
        )
        ...
        try:
            ...
            logger.info(f"Content saved to temporary file: {temp_file_path} ({file_size} bytes)")
