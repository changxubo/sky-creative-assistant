...
class RivaTTS:
    ...
    def list_voices(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for model_config in config_response.model_config:
                ...
                full_voice_names = [f"{voice_name}.{subvoice}" for subvoice in subvoices]
    ...
    def text_to_speech(
        ...
        if not text or not text.strip():
            return {"success": False, "error": "Input text is empty", "audio_data": None}
        ...
        try:
            ...
            if output_file is not None:
                try:
                    ...
                    return {"success": False, "error": f"Cannot open output file: {str(e)}", "audio_data": None}
            ...
            if audio_data is None or len(audio_data) == 0:
                ...
                return {"success": False, "error": "No audio data returned from TTS service", "audio_data": None}
            
            # Convert binary audio to base64 for API compatibility
            audio_data_base64 = base64.b64encode(audio_data).decode('utf-8')
            
            # Calculate audio duration
            audio_duration = len(audio_data) / (self.sample_rate_hz * sample_width * num_channels)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--function-id", 
        help="Function ID for cloud-based Riva."
    )
    parser.add_argument(
        "--authorization", 
